        ...
        <div className="p-8 space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="border-2 border-black p-3 bg-white">
                  ...
                  <p className="text-[11px] font-black text-emerald-600">{request.enrollment?.progress_percent || 0}%</p>
                </div>
              </div>
              ...
              {!isPending && (
                <div className="border-4 border-black p-4 bg-zinc-100 space-y-2">
                  <p className="text-[10px] font-black uppercase text-zinc-500">Thông tin xử lý</p>
                  <p className="text-xs font-black uppercase">Trạng thái: {request.status}</p>
                  <p className="text-xs font-black uppercase">
                    Xử lý lúc: {request.processed_at ? new Date(request.processed_at).toLocaleString('vi-VN') : '--'}
                  </p>
                  <p className="text-xs font-black uppercase">
                    Người xử lý: {request.processed_by?.full_name || request.processed_by?.email || request.processed_by_id || '--'}
                  </p>
                  {request.admin_note && (
                    <p className="text-xs font-black uppercase">Ghi chú: {request.admin_note}</p>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="pt-6 border-t-8 border-black border-double flex flex-col md:flex-row gap-4">
            {isPending ? (
              ...
              <div className="w-full bg-zinc-100 border-4 border-black p-5 text-center font-black uppercase text-xs">
                Yêu cầu này đã được xử lý, không thể thao tác lại.
              </div>
            )}
          </div>
        </div>
...
export default function AdminRefundsPage() {
  ...
  const handleProcess = async (
    ...
    if (!request || request.status !== 'PENDING') {
      toast('Yêu cầu này đã được xử lý trước đó.');
      return;
    }
      ...
      await processRefund(
        ...
        note ||
          (status === 'APPROVED'
            ...
            : 'Rất tiếc, yêu cầu hoàn tiền của bạn bị từ chối.'),
      );
      toast.success(status === 'APPROVED' ? 'Đã duyệt hoàn tiền!' : 'Đã từ chối yêu cầu.');
      setIsModalOpen(false);
      await fetchAllRefundRequests(filterStatus === 'ALL' ? undefined : filterStatus, dateFrom || undefined, dateTo || undefined);
    } catch (err) {
        ...
        <div className="flex flex-col gap-6 border-b-8 border-black pb-8">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <input
              ...
              className="w-full md:w-52 h-12 bg-white border-4 border-black px-4 font-black uppercase text-xs shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] outline-none"
            />
            <input
              type="date"
              value={dateTo}
            ...
            <button
              ...
              className="w-full md:w-56 h-12 bg-white border-4 border-black px-4 font-black uppercase text-xs shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-zinc-100 transition-colors"
            >
